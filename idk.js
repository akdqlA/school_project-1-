...
window.onload = function() {
    if(x < 1900 && y < 1000)
    {
        ...
        AAA.style.transform = "translate(-250px, 0px)";
        xn=997.5;
        yn=5;
    }
};
...
function choose(a){
    ...
    for(let i = 0 ; i < foods.length; i ++)
    {
        ...
        b.style.left = `${xn}px`;
        b.style.top = `${(i % 4) * yp + yn}px`;
        console.log((i % 4) * yp + yn);
        if(i > 3)
            b.style.left=`${xn+xp}px`;
        if(i > 7)
            b.style.left=`${xn+xp*2}px`;
...
function Result(){
    ...
    for(let i = 0 ; i < 3 ; i ++){
        ...
        if(List[i].length == 0){
            ...
            p1[i].innerText=`탄수화물:${res1}`;
            p2[i].innerText=`당${res2}`;
        }
    }
